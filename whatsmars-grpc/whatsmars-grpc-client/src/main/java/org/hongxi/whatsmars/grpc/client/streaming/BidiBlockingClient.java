package org.hongxi.whatsmars.grpc.client.streaming;
...
import org.hongxi.whatsmars.grpc.api.streaming.HelloReply;
import org.hongxi.whatsmars.grpc.api.streaming.HelloRequest;
import org.hongxi.whatsmars.grpc.api.streaming.StreamingGreeterGrpc;
import org.hongxi.whatsmars.grpc.api.streaming.StreamingGreeterGrpc.StreamingGreeterBlockingV2Stub;
...
public class BidiBlockingClient {
    private static final Logger logger = LoggerFactory.getLogger(BidiBlockingClient.class);
                    ...
                    public void run() {
                        ...
                        try {
                            while (stream.hasNext()) {
                                ...
                                if (++count % 10 == 0) {
                                    logger.info("Finished {} reads", count);
                                }
                            }
                        ...
                        try {
                            while (iterator.hasNext()) {
                                if (!stream.write(HelloRequest.newBuilder().setName(iterator.next()).setPadding(padding)
                                        .build())) {
                                    logger.warn("Stream closed before writes completed");
                                    hadProblem = true;
                                    break;
                                }
                                if (++count % 10 == 0) {
                                    logger.info("Finished {} writes", count);
                                }
                            }
                            if (!hadProblem) {
                            ...
                            logger.warn("Encountered error while writing: ", e);
                        }
                    }
                }, "writer");
