...
> 本模块基于 LangChain4j + Spring Boot Starter，使用阿里通义千问（Qwen）OpenAI 兼容接口。
...
LangChain4j 使用 `@AiService` 注解声明 AI 服务接口，类似 MyBatis 的 Mapper 接口。
...
使用 `TokenStream` 配合 Spring MVC 的 `SseEmitter` 实现流式输出。
...
使用 `@MemoryId` 注解实现多会话隔离，每个 sessionId 独立维护对话历史。

```java
@AiService
public interface ChatWithMemoryAssistant {
    @SystemMessage("你是一个专业的 Java 技术专家，能够记住之前的对话内容。")
...
public interface FunctionCallingAssistant {
    @SystemMessage("你是一个智能助手，可以使用提供的工具来帮助用户。")
    String chat(String userMessage);
}
```
...
├── StreamingAssistant.java          # 流式响应接口 (TokenStream)
├── StreamingController.java         # 流式控制器 (SSE)
├── ChatWithMemoryAssistant.java     # 多轮对话接口 (@MemoryId)
├── ChatWithMemoryController.java    # 多轮对话控制器
├── FunctionCallingAssistant.java    # 工具调用接口
├── FunctionCallingController.java   # 工具调用控制器
└── ToolService.java                 # 工具定义 (@Tool)
```
